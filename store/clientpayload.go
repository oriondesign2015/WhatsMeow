...
var DeviceProps = &waCompanionReg.DeviceProps{
	...
	HistorySyncConfig: &waCompanionReg.DeviceProps_HistorySyncConfig{
		...
		RecentSyncDaysLimit: proto.Uint32(0),  // Desabilita sincronização recente (resolve o problema!)
	},
}
...
// SetHistorySyncConfig permite configurar os limites de sincronização de histórico.
// Use 0 para desabilitar completamente a sincronização de histórico (recomendado para evitar bloqueio de mensagens novas).
// Use valores maiores (como 30, 90, 365) para limitar quantos dias de histórico sincronizar.
//
// Parâmetros:
//   - fullSyncDaysLimit: Limite de dias para sincronização completa (0 = desabilitado)
//   - recentSyncDaysLimit: Limite de dias para sincronização recente (0 = desabilitado)
//
// Exemplo:
//
//	store.SetHistorySyncConfig(0, 0) // Desabilita todo o histórico
//	store.SetHistorySyncConfig(30, 7) // Sincroniza até 30 dias (completo) e 7 dias (recente)
